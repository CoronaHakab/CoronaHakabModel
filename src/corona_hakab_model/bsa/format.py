...
from typing import Any, BinaryIO, Callable, Collection, Iterable, Optional, Sequence
...
class EncoderV0:
    ...
    def __init__(
        ...
        size: int,
        layer_count: int,
        indices: Sequence[Collection[int]],
        user_data: bytes = b"",
        flags: Flags = Flags(0),
    ):
        self.sink = sink
        ...
        # rows
        self._write_uints((len(r) for r in self.indices), 4)
        # columns
    ...
    def add_layer(self, dtype: BSA_Dtype, map_: Callable[[int, int], Any], user_data: bytes = b"", default=0):
        ...
        for i, row in enumerate(self.indices):
            arr.extend(map_(i, j) for j in row)
    ...
    def _write_uints(self, i: Iterable[int], chars: int):
        # todo improve?
        self._write_many((n.to_bytes(chars, "little") for n in i))
...
class DecoderV0:
    ...
    def __init__(self, source: BinaryIO, read_magic: bool):
        ...
        for i in range(self.layer_count):
            self.layers.append(self.Layer(self, i))
    ...
    def _read_uints(self, n: int, count: int):
        # todo improve?
        yield from (self._read_uint(n) for _ in range(count))
