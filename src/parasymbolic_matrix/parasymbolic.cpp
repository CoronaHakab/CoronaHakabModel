...
virtual dtype BareSparseMatrix::operator[](size_t row_ind, size_t column_ind{
...
void BareSparseMatrix::batch_set(size_t row_num,
    ...
    auto tup_index = 0;
    while (true){
        next_i_col = columns[tup_index];
        next_i_val = values[tup_index];
        if ((next_i_val == 0) && ++tup_index == c_len) return;

        while (*iter < next_i_col){
            if (++iter == row.end()) break;
        }
        if (iter == row.end()){
            for (i = tup_index; i < c_len; ++i){
        ...
        if (*iter == next_i_col){
            this.total += values[i] - *iter;
            *iter = next_i_val;
            if (++tup_index == c_len) return;
        }
        else { // *iter > next_i_col
...
virtual ~BareSparseMatrix::BareSparseMatrix(){
    delete[] rows;
    delete[] columns;
}
// endregion
...
dtype CoffedSparseMatrix::operator[](size_t row, size_t column){
    coffs = row_coefficients[row] * col_coefficients[column];
    if (coffs == 0)
        return 0;
    return base::operator[row, column] * coffs;
}
...
// endregion
// region parasymbolic
ParasymbolicMatrix::ParasymbolicMatrix(dtype const* factors, CoffedSparseMatrix const* comps, size_t len):
...
void ParasymbolicMatrix::rebuild_column(size_t col_num){
    ...
    for(auto row_num: col_set){
        dtype total = 0;
        for (auto comp_num = 0; comp_num < component_count; comp_num++){
            total += components[comp_num][row_num, col_num];
        }
...
ParasymbolicMatrix ParasymbolicMatrix::from_tuples(std::vector<std::tuple<CoffedSparseMatrix, dtype>> members){
    ...
    return ParasymbolicMatrix(comps, factors, members.size())
}

dtype ParasymbolicMatrix::operator[](size_t row, size_t column){
...
virtual ParasymbolicMatrix::~ParasymbolicMatrix(){
    delete components;
    delete factors;
}
// endregion